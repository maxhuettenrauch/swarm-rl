...
from swarm_zoo.point_envs.rendezvous import RendezvousEnv, PyGObsWrapper
...
if __name__ == '__main__':
    ...
    env = RendezvousEnv(num_agents=4, render_mode='lh')
    env = ss.pettingzoo_env_to_vec_env_v1(env)
    env = PyGObsWrapper(env)
    env = ss.concat_vec_envs_v1(env, 4, 0, base_class='gymnasium')

    policy = GraphEmbeddingPolicy(env.observation_space, env.action_space, lambda t: 0.01,
